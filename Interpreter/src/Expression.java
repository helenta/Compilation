...
//Can hold var OR number OR BinaryOP for to integers
{
    ...
    {
    	//Receive the PARTIAL string of the expression. The first char should not be whitespace!
    ...
    //*** should run when we want to run syntax error. Sending dummy values to execute, in order to check the syntax of the expression    ***/
    ...
    //should be called from the Command class using the m_token that was created in the constructor
    ...
    private int Execute(PolishProgram program, String[] token)
    {
    	...
    	for (int i = token.length-1; i>=0; i--)
        {
    		if (isVariable(token[i]))
    			...
    			if (value == -1){
    				program.AddError(PolishProgram.ErrorCodeInitalization, "The '" + token[i].charAt(0) + "' variable is not assigned.");
    			}
    		...
    		else if(expressionOp(token[i]) != BinaryOp.None)
                {
    			result = 0;
    			try
                        {
    				result = processOperator(expressionOp(token[i]),literalStack.pop(),literalStack.pop());
    			}
    			catch (EmptyStackException e)
                        {
    				program.AddError(PolishProgram.ErrorCodeGrammer, "expression syntax error ");
    				return -1;
    			}
                ...
                {
    			program.AddError(PolishProgram.ErrorCodeGrammer, "expression syntax error - illegal term"); 
    			return -1;
    		}
        ...
        {
    		program.AddError(PolishProgram.ErrorCodeGrammer, "expression syntax error - stack isnt empty "); 
    		return -1;
    	}
    	return result;
    ...
    private int processOperator(BinaryOp expressionOp, Integer pop, Integer pop2) {
		...
		else if (expressionOp == BinaryOp.minus){
			return pop-pop2;
		}
		else if (expressionOp == BinaryOp.mult){
			return pop*pop2;
		}
		else if (expressionOp == BinaryOp.devide){
			return pop/pop2;
		}
		return -1;
	}
