class Command
{
	...
	{
		// Receive the PARTIAL string - without the label and the ':'. The first
		// char should not be whitespace!
		...
		if (strCommand.startsWith("print"))
		{
			...
			if (strCommand.startsWith("if"))
			{
				...
				{
					program.AddError(PolishProgram.ErrorCodeGrammer,
					    "Exprcted too vars and boolean op");
					return;
				}
				...
				if (boolOp == '<' || boolOp == '>')
				{
					...
					{
						m_BoolOp = boolOp == '<' ? BoolOp.smallerEquals
						    : BoolOp.biggerEquals;
						m_VarName2 = third;
					}
					else
						if (StringUtils.IsEnglishChar(next) && third == 0)
						{
						...
						{
							program.AddError(PolishProgram.ErrorCodeGrammer,
							    "Unable to parse bool op 1");
							return;
						}
				}
				else
					if ((boolOp == '=' || boolOp == '!')
					    && (next == '=' && StringUtils.IsEnglishChar(third)))
					{
				...
				if (strCommand.startsWith("goto"))
				{
					...
					{
						program
						    .AddError(PolishProgram.ErrorCodeGrammer, "Unable parse int");
						return;
					}
					else
						if (!program.LabelExists(lable))
						{
							program.AddError(PolishProgram.ErrorCodeGoto,
							    "Wrong lable in goto op.");
							return;
						}
						else
						{
				...
				// try to create assignment command
				{
					int index = strCommand.indexOf(":=");
					if (index == -1) // check if we have only one letter and := after
					                 // that
					{
						program.AddError(PolishProgram.ErrorCodeGrammer,
						    "Expected ':=' for assignement op.");
						return;
					}
	...
	private static boolean EveluateBoolOpearion(int val1, int val2, BoolOp boolOp)
	    throws Exception
	{
		switch (boolOp)
		{
